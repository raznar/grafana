...
export function ShareModal({ dashboard, panel, activeTab: initialActiveTab, onDismiss }: Props) {
  const initialState = useMemo(() => {
    const { tabs, activeTab } = getTabs(dashboard.canEditDashboard(), panel, initialActiveTab);
    return { tabs, activeTab };
  }, [dashboard, panel, initialActiveTab]);

  const [activeTab, setActiveTab] = useState(initialState.activeTab);

  const onSelectTab: ComponentProps<typeof ModalTabsHeader>['onChangeTab'] = useCallback(
    (t) => {
  ...
  const activeTabModel = useMemo(() => {
    return initialState.tabs.find((t) => t.value === activeTab)!;
  }, [initialState.tabs, activeTab]);
