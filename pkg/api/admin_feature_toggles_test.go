...
func TestAPI_AdminFeatureToggles(t *testing.T) {
	...
	t.Run("rejects missing enabled values", func(t *testing.T) {
		server, manager := setupAdminFeatureTogglesTestServer(t)

		res, err := server.SendJSON(webtest.RequestWithSignedInUser(
			server.NewRequest(http.MethodPut, "/api/admin/feature-toggles", bytes.NewBufferString(`{"toggles":[{"name":"featureHighlights"}]}`)),
			grafanaAdminUser(),
		))
		require.NoError(t, err)
		defer func() {
		...
		require.Equal(t, http.StatusBadRequest, res.StatusCode)
		assert.False(t, manager.IsEnabled(context.Background(), "featureHighlights"))
	})
	...
	t.Run("rejects invalid batches without applying valid changes", func(t *testing.T) {
		server, manager := setupAdminFeatureTogglesTestServer(t)

		res, err := server.SendJSON(webtest.RequestWithSignedInUser(
			server.NewRequest(http.MethodPut, "/api/admin/feature-toggles", bytes.NewBufferString(`{"toggles":[{"name":"featureHighlights","enabled":true},{"name":"liveAPIServer","enabled":true}]}`)),
			grafanaAdminUser(),
		))
		require.NoError(t, err)
		defer func() {
